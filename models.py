...
class LENET(BaseModel):
    def __init__(self, config):
        ...
        self.passcnt   = 0
        self.config    = config
        self.cnn1      = Conv2dBNN(1,   6  , kernel_size=(5, 5), padding=2, H=1, W_LR_scale="Glorot", config=config)
        self.cnn2      = Conv2dBNN(6,   16 , kernel_size=(5, 5), padding=2, H=1, W_LR_scale="Glorot", config=config)
        self.cnn3      = Conv2dBNN(16,  120, kernel_size=(5, 5), padding=2, H=1, W_LR_scale="Glorot", config=config)

        self.fc1       = LinearBNN(7*7*120, 120, H=1, W_LR_scale="Glorot", config=config)
        self.fc2       = LinearBNN(120, 84, H=1, W_LR_scale="Glorot"     , config=config)
